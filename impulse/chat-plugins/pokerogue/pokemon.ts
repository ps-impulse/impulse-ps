...
export function expForLevel(level: number, expType = 'Medium Fast'): number {
        ...
        switch (expType) {
        case 'Erratic':
                raw = (Math.pow(level, 4) + Math.pow(level, 3) * 2000) / 3500;
                break;
        case 'Fast':
                raw = (Math.pow(level, 3) * 4) / 5;
                break;
        case 'Medium Fast':
                return Math.floor(Math.pow(level, 3));
        case 'Medium Slow':
                raw = (Math.pow(level, 3) * 6) / 5 - 15 * Math.pow(level, 2) + 100 * level - 140;
                break;
        case 'Slow':
                raw = (Math.pow(level, 3) * 5) / 4;
                break;
        case 'Fluctuating':
                raw = (Math.pow(level, 3) * (level / 2 + 8) * 4) / (100 + level);
                break;
        default:
                return Math.floor(Math.pow(level, 3));
        }
...
export function levelScaleForFloor(floor: number): [number, number] {
    ...
    const baseLevel = (1 + waveIndex / 2 + Math.pow(waveIndex / 25, 2)) * 1.2;
    ...
    const currentBase = (1 + floor / 2 + Math.pow(floor / 25, 2)) * 1.2;
