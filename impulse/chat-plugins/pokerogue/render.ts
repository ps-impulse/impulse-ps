...
function getSprite(species: string, size = 80): string {
        ...
        const onerror = ` onerror="this.onerror=function(){this.onerror=null;this.style.display='none'};this.src='${fallback}'"`;
...
function renderMainView(state: PokeRogueState, user: User): string {
        ...
        let buf = renderStatBar(state) + `<div class="pr-section-title">Your team</div>`;
        ...
        buf += `</tbody></table></div><div class="pr-actions">`;
        ...
        buf += renderBtn('/pokerogue view top', 'Ladder');
        buf += renderBtn('/pokerogue view guide', 'Guide');
...
function renderHeader(view: string, hasGameOver: boolean): string {
        ...
        if (view !== 'main' && !hasGameOver) buf += renderBtn('/pokerogue view main', '← Back', 'pr-btn', 'font-size:11px;padding:5px 10px');
        return buf + `</div>`;
}
...
function renderGuideView(): string {
        ...
        buf += renderGuidePanel(`TMs appear in the rotational shop. Buying a TM opens a Pokémon selection screen — only Pokémon that can legally learn the move are enabled. If the target already has 4 moves, you enter the move-replacement flow: choose a slot to overwrite or skip. Cancelling the TM before assigning it to any Pokémon gives a full BP refund.`);
        ...
        buf += `<div class="pr-section-title">AI difficulty scaling</div>`;
        buf += renderGuidePanel(`The enemy AI grows meaningfully stronger as you climb:<ul style="margin:5px 0 0;padding-left:16px"><li><b>Floors 1–10</b> — random IVs (0–31), minimal EVs (0–40 per stat), random natures. Beatable with any team.</li><li><b>Floors 11–20</b> — IVs 15–31, balanced 40 EVs per stat.</li><li><b>Floors 21–30</b> — IVs 20–31, full 510 EV spread (85 per stat).</li><li><b>Floors 31–40</b> — perfect 31 IVs, 510 EVs.</li><li><b>Floors 41+</b> — perfect IVs, optimised 252/252/6 EV spread targeting the two highest base stats, with an appropriate speed or attack nature (Jolly/Adamant/Timid/Modest).</li></ul>The AI also scores move type effectiveness against your active Pokémon, prioritises sleep, paralysis, burn, and toxic moves, and has a ~15% chance to switch in a counter each turn.`);
